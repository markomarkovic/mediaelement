...
0,p=mejs.players.length;h<p;h++){var j=mejs.players[h];j.id!=c.id&&c.options.pauseOtherPlayers&&!j.paused&&!j.ended&&j.pause();j.hasFocus=false}c.hasFocus=true},false);c.media.addEventListener("ended",function(){try{c.media.setCurrentTime(0)}catch(h){}c.media.pause();c.setProgressRail&&c.setProgressRail();c.setCurrentRail&&c.setCurrentRail();if(c.options.loop)c.media.play();else!c.options.alwaysShowControls&&c.controlsEnabled&&c.showControls()},false);c.media.addEventListener("loadedmetadata",function(){c.updateDuration&&
...
g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),i=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){e.paused?e.play():e.pause()});e.addEventListener("play",function(){i.hide();d.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("playing",function(){i.hide();d.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("seeking",
function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("seeked",function(){d.hide();b.find(".mejs-time-buffering").hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||i.show()},false);e.addEventListener("waiting",function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("loadeddata",function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("canplay",function(){d.hide();b.find(".mejs-time-buffering").hide()},
false);e.addEventListener("error",function(){d.hide();b.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,e){f(document).keydown(function(d){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,i=a.options.keyActions.length;g<i;g++)for(var h=a.options.keyActions[g],p=0,j=h.keys.length;p<j;p++)if(d.keyCode==h.keys[p]){d.preventDefault();h.action(a,e,d.keyCode);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==
...
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var g=b.find(".mejs-time-current"),i=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),p=b.find(".mejs-time-float-current"),j=function(m){m=m.pageX;var r=d.offset(),k=d.outerWidth(),l=0;l=0;var n=m-r.left;if(m>r.left&&m<=k+r.left&&e.duration){l=(m-r.left)/k;l=l<=0.02?0:l*e.duration;o&&e.setCurrentTime(l);if(!mejs.MediaFeatures.hasTouch){h.css("left",n);p.html(mejs.Utility.secondsToTimeCode(l));h.show()}}},o=false;d.bind("mousedown",function(m){if(m.which===
1){o=true;j(m);f(document).bind("mousemove.dur",function(r){j(r)}).bind("mouseup.dur",function(){o=false;h.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(m){j(m)});mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){if(!o){f(document).unbind(".dur");h.hide()}});e.addEventListener("progress",function(m){a.setProgressRail(m);a.setCurrentRail(m)},false);e.addEventListener("timeupdate",function(m){a.setProgressRail(m);
a.setCurrentRail(m)},false);this.loaded=c;this.total=d;this.current=g;this.handle=i},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==null){c=Math.min(1,Math.max(0,c));this.loaded&&this.total&&this.loaded.width(this.total.width()*
...
i=this.container.find(".mejs-volume-slider, .mejs-horizontal-volume-slider"),h=this.container.find(".mejs-volume-total, .mejs-horizontal-volume-total"),p=this.container.find(".mejs-volume-current, .mejs-horizontal-volume-current"),j=this.container.find(".mejs-volume-handle, .mejs-horizontal-volume-handle"),o=function(l,n){if(!i.is(":visible")&&typeof n==="undefined"){i.show();o(l,true);i.hide()}else{l=Math.max(0,l);l=Math.min(l,1);l==0?g.removeClass("mejs-mute").addClass("mejs-unmute"):g.removeClass("mejs-unmute").addClass("mejs-mute");
if(d=="vertical"){var s=h.height(),t=h.position(),u=s-s*l;j.css(Math.round("top",t.top+u-j.height()/2));p.height(Math.round(s-u));p.css("top",Math.round(t.top+u))}else{s=h.width();t=h.position();s=s*l;j.css("left",Math.round(t.left+s-j.width()/2));p.width(Math.round(s))}}},m=function(l){var n=null,s=h.offset();if(d=="vertical"){n=h.height();parseInt(h.css("top").replace(/px/,""),10);n=(n-(l.pageY-s.top))/n;if(s.top==0||s.left==0)return}else{n=h.width();n=(l.pageX-s.left)/n}n=Math.max(0,n);n=Math.min(n,
1);o(n);n==0?e.setMuted(true):e.setMuted(false);e.setVolume(n)},r=false,k=false;g.hover(function(){i.show();k=true},function(){k=false;!r&&d=="vertical"&&i.hide()});i.bind("mouseover",function(){k=true}).bind("mousedown",function(l){m(l);f(document).bind("mousemove.vol",function(n){m(n)}).bind("mouseup.vol",function(){r=false;f(document).unbind(".vol");!k&&d=="vertical"&&i.hide()});r=true;return false});g.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){if(!r)if(e.muted){o(0);
g.removeClass("mejs-mute").addClass("mejs-unmute")}else{o(e.volume);g.removeClass("mejs-unmute").addClass("mejs-mute")}},false);if(this.container.is(":visible")){o(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var i=null;if(function(){var k=document.createElement("x"),l=document.documentElement,n=window.getComputedStyle;if(!("pointerEvents"in k.style))return false;k.style.pointerEvents="auto";k.style.pointerEvents="x";l.appendChild(k);n=n&&n(k,"").pointerEvents==="auto";l.removeChild(k);return!!n}()&&!mejs.MediaFeatures.isOpera){var h=false,p=function(){if(h){j.hide();o.hide();m.hide();g.css("pointer-events","");d.controls.css("pointer-events",
"");h=false}},j=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(p),o=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(p),m=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(p),r=function(){var k={position:"absolute",top:0,left:0};j.css(k);o.css(k);m.css(k);j.width(d.container.width()).height(d.container.height()-d.controls.height());k=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);o.width(k).height(d.controls.height()).css({top:d.container.height()-
d.controls.height()});m.width(d.container.width()-k-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:k+fullScreenBtnWidth})};f(document).resize(function(){r()});g.mouseover(function(){if(!d.isFullScreen){var k=g.offset(),l=a.container.offset();e.positionFullscreenButton(k.left-l.left,k.top-l.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");j.show();m.show();o.show();r();h=true}});e.addEventListener("fullscreenchange",
function(){p()})}else g.mouseover(function(){if(i!==null){clearTimeout(i);delete i}var k=g.offset(),l=a.container.offset();e.positionFullscreenButton(k.left-l.left,k.top-l.top,true)}).mouseout(function(){if(i!==null){clearTimeout(i);delete i}i=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(k){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&k.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
...
if(!d.start&&c.eq(b-1).attr("end"))d.start=mejs.Utility.convertSMPTEtoSeconds(c.eq(b-1).attr("end"));if(c.eq(b).attr("end"))d.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b).attr("end"));if(!d.stop&&c.eq(b+1).attr("begin"))d.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b+1).attr("begin"));if(e){g="";for(var i in e)g+=i+":"+e[i]+";"}if(g)d.style=g;if(d.start==0)d.start=0.2;a.times.push(d);d=f.trim(c.eq(b).html()).replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig,
...
true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,b){for(var c=this,e="",d=c.options.contextMenuItems,g=0,i=d.length;g<
i;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var h=d[g].render(c);if(h!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+h+"</div>"}c.contextMenu.empty().append(f(e)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var p=f(this),j=parseInt(p.data("itemindex"),10),o=c.options.contextMenuItems[j];typeof o.show!="undefined"&&o.show(p,c);p.click(function(){typeof o.click!=
"undefined"&&o.click(c);c.contextMenu.hide()})});setTimeout(function(){c.killControlsTimer("rev3")},100)}})})(mejs.$);
(function(f){f.extend(mejs.MepDefaults,{postrollCloseText:mejs.i18n.t("Close")});f.extend(MediaElementPlayer.prototype,{buildpostroll:function(a,b,c){var e=this.container.find('link[rel="postroll"]').attr("href");if(typeof e!=="undefined"){a.postroll=f('<div class="mejs-postroll-layer mejs-layer"><a class="mejs-postroll-close" onclick="$(this).parent().hide();return false;">'+this.options.postrollCloseText+'</a><div class="mejs-postroll-layer-content"></div></div>').prependTo(c).hide();this.media.addEventListener("ended",
function(){f.ajax({dataType:"html",url:e,success:function(d){c.find(".mejs-postroll-layer-content").html(d)}});a.postroll.show()},false)}}})})(mejs.$);
(function(f){function a(b,c,e){for(var d in c.type){q=c.quality[d];if(q===e){b.pause();b.setSrc(c.src[d])}}}f.extend(mejs.MepDefaults,{hdsdText:"HD/SD"});f.extend(MediaElementPlayer.prototype,{buildhdsd:function(b,c,e,d){var g="",i="",h=0,p=true,j={type:[],quality:[],src:[]},o=f('<div class="mejs-button mejs-hd-button mejs-sd" ><button type="button" aria-controls="'+this.id+'" title="'+this.options.hdsdText+'"></button></div>').appendTo(c).delegate(".mejs-hd-button > button","click",function(){i=
d.currentSrc;p=d.paused;h=d.currentTime;for(var m in d.children){src=d.children[m];if(src.nodeName==="SOURCE"&&src.src==i)g=f(src).attr("type")}if(g!==""&&!j.src.length)for(m in d.children){src=d.children[m];if(src.nodeName==="SOURCE"&&f(src).attr("type")==g&&j.src.indexOf(f(src).attr("src"))&&(d.canPlayType(src.type)=="probably"||d.canPlayType(src.type)=="maybe")){j.type.push(f(src).attr("type"));j.src.push(f(src).attr("src"));f(src).attr("data-quality")?j.quality.push(f(src).attr("data-quality")):
j.quality.push("sd")}}if(o.hasClass("mejs-sd")){a(d,j,"hd");o.removeClass("mejs-sd").addClass("mejs-hd")}else{a(d,j,"sd");o.removeClass("mejs-hd").addClass("mejs-sd")}setTimeout(function(){p||f(".mejs-playpause-button").click();d.addEventListener("loadeddata",function(){d.setCurrentTime(h)},false)},250)})}})})(mejs.$);
