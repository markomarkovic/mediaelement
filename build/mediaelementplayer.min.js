...
if(d=="vertical"){var s=h.height(),t=h.position(),u=s-s*l;j.css("top",t.top+u-j.height()/2);p.height(s-u);p.css("top",t.top+u)}else{s=h.width();t=h.position();s=s*l;j.css("left",t.left+s-j.width()/2);p.width(s)}}},m=function(l){var n=null,s=h.offset();if(d=="vertical"){n=h.height();parseInt(h.css("top").replace(/px/,""),10);n=(n-(l.pageY-s.top))/n;if(s.top==0||s.left==0)return}else{n=h.width();n=(l.pageX-s.left)/n}n=Math.max(0,n);n=Math.min(n,1);o(n);n==0?e.setMuted(true):e.setMuted(false);e.setVolume(n)},
r=false,k=false;g.hover(function(){i.show();k=true},function(){k=false;!r&&d=="vertical"&&i.hide()});i.bind("mouseover",function(){k=true}).bind("mousedown",function(l){m(l);f(document).bind("mousemove.vol",function(n){m(n)}).bind("mouseup.vol",function(){r=false;f(document).unbind(".vol");!k&&d=="vertical"&&i.hide()});r=true;return false});g.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){if(!r)if(e.muted){o(0);g.removeClass("mejs-mute").addClass("mejs-unmute")}else{o(e.volume);
g.removeClass("mejs-unmute").addClass("mejs-mute")}},false);if(this.container.is(":visible")){o(a.options.startVolume);e.pluginType==="native"&&e.setVolume(a.options.startVolume)}}}})})(mejs.$);
