...
0,p=mejs.players.length;h<p;h++){var o=mejs.players[h];o.id!=c.id&&c.options.pauseOtherPlayers&&!o.paused&&!o.ended&&o.pause();o.hasFocus=false}c.hasFocus=true},false);c.media.addEventListener("ended",function(){try{c.media.setCurrentTime(0)}catch(h){}c.media.pause();c.setProgressRail&&c.setProgressRail();c.setCurrentRail&&c.setCurrentRail();if(c.options.loop)c.media.play();else!c.options.alwaysShowControls&&c.controlsEnabled&&c.showControls()},false);c.media.addEventListener("loadedmetadata",function(){c.updateDuration&&
...
this.options.defaultVideoWidth:this.options.defaultAudioWidth,d=this.isVideo?this.media.videoHeight&&this.media.videoHeight>0?this.media.videoHeight:this.options.defaultVideoHeight:this.options.defaultAudioHeight,e=this.container.parent().closest(":visible").width();c=this.isVideo||!this.options.autosizeProgress?parseInt(e*d/c,10):d;if(this.container.parent()[0].tagName.toLowerCase()==="body"){e=f(window).width();c=f(window).height()}if(c!=0&&e!=0){this.container.width(e).height(c);this.$media.width("100%").height("100%");
...
g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),l=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){d.paused?d.play():d.pause()});d.addEventListener("play",function(){l.hide();e.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);d.addEventListener("playing",function(){l.hide();e.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);d.addEventListener("seeking",
function(){e.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("seeked",function(){e.hide();b.find(".mejs-time-buffering").hide()},false);d.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||l.show()},false);d.addEventListener("waiting",function(){e.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("loadeddata",function(){e.show();b.find(".mejs-time-buffering").show()},false);d.addEventListener("canplay",function(){e.hide();b.find(".mejs-time-buffering").hide()},
false);d.addEventListener("error",function(){e.hide();b.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,d){f(document).keydown(function(e){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,l=a.options.keyActions.length;g<l;g++)for(var h=a.options.keyActions[g],p=0,o=h.keys.length;p<o;p++)if(e.keyCode==h.keys[p]){e.preventDefault();h.action(a,d,e.keyCode);return false}return true});f(document).click(function(e){if(f(e.target).closest(".mejs-container").length==
...
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var g=b.find(".mejs-time-current"),l=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),p=b.find(".mejs-time-float-current"),o=function(m){m=m.pageX;var r=e.offset(),j=e.outerWidth(),k=0;k=0;var n=m-r.left;if(m>r.left&&m<=j+r.left&&d.duration){k=(m-r.left)/j;k=k<=0.02?0:k*d.duration;q&&d.setCurrentTime(k);if(!mejs.MediaFeatures.hasTouch){h.css("left",n);p.html(mejs.Utility.secondsToTimeCode(k));h.show()}}},q=false;e.bind("mousedown",function(m){if(m.which===
1){q=true;o(m);f(document).bind("mousemove.dur",function(r){o(r)}).bind("mouseup.dur",function(){q=false;h.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(m){o(m)});mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){if(!q){f(document).unbind(".dur");h.hide()}});d.addEventListener("progress",function(m){a.setProgressRail(m);a.setCurrentRail(m)},false);d.addEventListener("timeupdate",function(m){a.setProgressRail(m);
a.setCurrentRail(m)},false);this.loaded=c;this.total=e;this.current=g;this.handle=l},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==null){c=Math.min(1,Math.max(0,c));this.loaded&&this.total&&this.loaded.width(this.total.width()*
...
l=this.container.find(".mejs-volume-slider, .mejs-horizontal-volume-slider"),h=this.container.find(".mejs-volume-total, .mejs-horizontal-volume-total"),p=this.container.find(".mejs-volume-current, .mejs-horizontal-volume-current"),o=this.container.find(".mejs-volume-handle, .mejs-horizontal-volume-handle"),q=function(k,n){if(!l.is(":visible")&&typeof n==="undefined"){l.show();q(k,true);l.hide()}else{k=Math.max(0,k);k=Math.min(k,1);k==0?g.removeClass("mejs-mute").addClass("mejs-unmute"):g.removeClass("mejs-unmute").addClass("mejs-mute");
if(e=="vertical"){var s=h.height(),t=h.position(),u=s-s*k;o.css(Math.round("top",t.top+u-o.height()/2));p.height(Math.round(s-u));p.css("top",Math.round(t.top+u))}else{s=h.width();t=h.position();s=s*k;o.css("left",Math.round(t.left+s-o.width()/2));p.width(Math.round(s))}}},m=function(k){var n=null,s=h.offset();if(e=="vertical"){n=h.height();parseInt(h.css("top").replace(/px/,""),10);n=(n-(k.pageY-s.top))/n;if(s.top==0||s.left==0)return}else{n=h.width();n=(k.pageX-s.left)/n}n=Math.max(0,n);n=Math.min(n,
1);q(n);n==0?d.setMuted(true):d.setMuted(false);d.setVolume(n)},r=false,j=false;g.hover(function(){l.show();j=true},function(){j=false;!r&&e=="vertical"&&l.hide()});l.bind("mouseover",function(){j=true}).bind("mousedown",function(k){m(k);f(document).bind("mousemove.vol",function(n){m(n)}).bind("mouseup.vol",function(){r=false;f(document).unbind(".vol");!j&&e=="vertical"&&l.hide()});r=true;return false});g.find("button").click(function(){d.setMuted(!d.muted)});d.addEventListener("volumechange",function(){if(!r)if(d.muted){q(0);
g.removeClass("mejs-mute").addClass("mejs-unmute")}else{q(d.volume);g.removeClass("mejs-unmute").addClass("mejs-mute")}},false);if(this.container.is(":visible")){q(a.options.startVolume);d.pluginType==="native"&&d.setVolume(a.options.startVolume)}}}})})(mejs.$);
...
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var l=null;if(function(){var j=document.createElement("x"),k=document.documentElement,n=window.getComputedStyle;if(!("pointerEvents"in j.style))return false;j.style.pointerEvents="auto";j.style.pointerEvents="x";k.appendChild(j);n=n&&n(j,"").pointerEvents==="auto";k.removeChild(j);return!!n}()&&!mejs.MediaFeatures.isOpera){var h=false,p=function(){if(h){o.hide();q.hide();m.hide();g.css("pointer-events","");e.controls.css("pointer-events",
"");h=false}},o=f('<div class="mejs-fullscreen-hover" />').appendTo(e.container).mouseover(p),q=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(p),m=f('<div class="mejs-fullscreen-hover"  />').appendTo(e.container).mouseover(p),r=function(){var j={position:"absolute",top:0,left:0};o.css(j);q.css(j);m.css(j);o.width(e.container.width()).height(e.container.height()-e.controls.height());j=g.offset().left-e.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);q.width(j).height(e.controls.height()).css({top:e.container.height()-
e.controls.height()});m.width(e.container.width()-j-fullScreenBtnWidth).height(e.controls.height()).css({top:e.container.height()-e.controls.height(),left:j+fullScreenBtnWidth})};f(document).resize(function(){r()});g.mouseover(function(){if(!e.isFullScreen){var j=g.offset(),k=a.container.offset();d.positionFullscreenButton(j.left-k.left,j.top-k.top,false);g.css("pointer-events","none");e.controls.css("pointer-events","none");o.show();m.show();q.show();r();h=true}});d.addEventListener("fullscreenchange",
function(){p()})}else g.mouseover(function(){if(l!==null){clearTimeout(l);delete l}var j=g.offset(),k=a.container.offset();d.positionFullscreenButton(j.left-k.left,j.top-k.top,true)}).mouseout(function(){if(l!==null){clearTimeout(l);delete l}l=setTimeout(function(){d.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(j){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||e.isFullScreen)&&j.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
...
true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,b){for(var c=this,d="",e=c.options.contextMenuItems,g=0,l=e.length;g<
l;g++)if(e[g].isSeparator)d+='<div class="mejs-contextmenu-separator"></div>';else{var h=e[g].render(c);if(h!=null)d+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+h+"</div>"}c.contextMenu.empty().append(f(d)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var p=f(this),o=parseInt(p.data("itemindex"),10),q=c.options.contextMenuItems[o];typeof q.show!="undefined"&&q.show(p,c);p.click(function(){typeof q.click!=
"undefined"&&q.click(c);c.contextMenu.hide()})});setTimeout(function(){c.killControlsTimer("rev3")},100)}})})(mejs.$);
...
(function(f){f.extend(mejs.MepDefaults,{sourcechooserText:"Source Chooser"});f.extend(MediaElementPlayer.prototype,{buildsourcechooser:function(a,b,c,d){a.sourcechooserButton=f('<div class="mejs-button mejs-sourcechooser-button"><button type="button" aria-controls="'+this.id+'" title="'+this.options.sourcechooserText+'"></button><div class="mejs-sourcechooser-selector"><ul></ul></div></div>').appendTo(b).hover(function(){f(this).find(".mejs-sourcechooser-selector").css("visibility","visible")},function(){f(this).find(".mejs-sourcechooser-selector").css("visibility",
"hidden")}).delegate("input[type=radio]","click",function(){src=this.value;if(d.currentSrc!=src){currentTime=d.currentTime;paused=d.paused;d.setSrc(src);paused||d.play()}});for(i in d.children){src=d.children[i];if(src.nodeName==="SOURCE"&&(d.canPlayType(src.type)=="probably"||d.canPlayType(src.type)=="maybe"))a.addSourceButton(src.src,src.title,src.type,d.src==src.src)}},addSourceButton:function(a,b,c,d){if(b===""||b==undefined)b=a;c=c.split("/")[1];this.sourcechooserButton.find("ul").append(f('<li><input type="radio" name="'+
this.id+'_sourcechooser" id="'+this.id+"_sourcechooser_"+b+c+'" value="'+a+'" '+(d?'checked="checked"':"")+' /><label for="'+this.id+"_sourcechooser_"+b+c+'">'+b+" ("+c+")</label></li>"));this.adjustSourcechooserBox()},adjustSourcechooserBox:function(){this.sourcechooserButton.find(".mejs-sourcechooser-selector").height(this.sourcechooserButton.find(".mejs-sourcechooser-selector ul").outerHeight(true))}})})(mejs.$);
(function(f){f.extend(mejs.MepDefaults,{googleAnalyticsTitle:"",googleAnalyticsCategory:"Videos",googleAnalyticsEventPlay:"Play",googleAnalyticsEventPause:"Pause",googleAnalyticsEventEnded:"Ended",googleAnalyticsEventTime:"Time"});f.extend(MediaElementPlayer.prototype,{buildgoogleanalytics:function(a,b,c,d){d.addEventListener("play",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventPlay,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+
1)];_gaq.push(ev)}},false);d.addEventListener("pause",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventPause,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+1)];_gaq.push(ev)}},false);d.addEventListener("ended",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventEnded,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+1)];_gaq.push(ev)}},
false)}})})(mejs.$);
