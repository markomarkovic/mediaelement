...
this.options.defaultVideoWidth:this.options.defaultAudioWidth,e=this.isVideo?this.media.videoHeight&&this.media.videoHeight>0?this.media.videoHeight:this.options.defaultVideoHeight:this.options.defaultAudioHeight,d=this.container.parent().closest(":visible").width();c=this.isVideo||!this.options.autosizeProgress?parseInt(d*e/c,10):e;if(this.container.parent()[0].tagName.toLowerCase()==="body"){d=f(window).width();c=f(window).height()}if(c!=0&&d!=0){this.container.width(d).height(c);this.$media.width("100%").height("100%");
...
g=f('<div class="mejs-overlay mejs-layer"><div class="mejs-overlay-error"></div></div>').hide().appendTo(c),j=f('<div class="mejs-overlay mejs-layer mejs-overlay-play"><div class="mejs-overlay-button"></div></div>').appendTo(c).click(function(){e.paused?e.play():e.pause()});e.addEventListener("play",function(){j.hide();d.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("playing",function(){j.hide();d.hide();b.find(".mejs-time-buffering").hide();g.hide()},false);e.addEventListener("seeking",
function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("seeked",function(){d.hide();b.find(".mejs-time-buffering").hide()},false);e.addEventListener("pause",function(){mejs.MediaFeatures.isiPhone||j.show()},false);e.addEventListener("waiting",function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("loadeddata",function(){d.show();b.find(".mejs-time-buffering").show()},false);e.addEventListener("canplay",function(){d.hide();b.find(".mejs-time-buffering").hide()},
false);e.addEventListener("error",function(){d.hide();b.find(".mejs-time-buffering").hide();g.show();g.find("mejs-overlay-error").html("Error loading this resource")},false)}},buildkeyboard:function(a,b,c,e){f(document).keydown(function(d){if(a.hasFocus&&a.options.enableKeyboard)for(var g=0,j=a.options.keyActions.length;g<j;g++)for(var h=a.options.keyActions[g],r=0,k=h.keys.length;r<k;r++)if(d.keyCode==h.keys[r]){d.preventDefault();h.action(a,e,d.keyCode);return false}return true});f(document).click(function(d){if(f(d.target).closest(".mejs-container").length==
...
b.find(".mejs-time-total");c=b.find(".mejs-time-loaded");var g=b.find(".mejs-time-current"),j=b.find(".mejs-time-handle"),h=b.find(".mejs-time-float"),r=b.find(".mejs-time-float-current"),k=function(n){n=n.pageX;var s=d.offset(),l=d.outerWidth(),m=0;m=0;var o=n-s.left;if(n>s.left&&n<=l+s.left&&e.duration){m=(n-s.left)/l;m=m<=0.02?0:m*e.duration;p&&e.setCurrentTime(m);if(!mejs.MediaFeatures.hasTouch){h.css("left",o);r.html(mejs.Utility.secondsToTimeCode(m));h.show()}}},p=false;d.bind("mousedown",function(n){if(n.which===
1){p=true;k(n);f(document).bind("mousemove.dur",function(s){k(s)}).bind("mouseup.dur",function(){p=false;h.hide();f(document).unbind(".dur")});return false}}).bind("mouseenter",function(){f(document).bind("mousemove.dur",function(n){k(n)});mejs.MediaFeatures.hasTouch||h.show()}).bind("mouseleave",function(){if(!p){f(document).unbind(".dur");h.hide()}});e.addEventListener("progress",function(n){a.setProgressRail(n);a.setCurrentRail(n)},false);e.addEventListener("timeupdate",function(n){a.setProgressRail(n);
a.setCurrentRail(n)},false);this.loaded=c;this.total=d;this.current=g;this.handle=j},setProgressRail:function(a){var b=a!=undefined?a.target:this.media,c=null;if(b&&b.buffered&&b.buffered.length>0&&b.buffered.end&&b.duration)c=b.buffered.end(0)/b.duration;else if(b&&b.bytesTotal!=undefined&&b.bytesTotal>0&&b.bufferedBytes!=undefined)c=b.bufferedBytes/b.bytesTotal;else if(a&&a.lengthComputable&&a.total!=0)c=a.loaded/a.total;if(c!==null){c=Math.min(1,Math.max(0,c));this.loaded&&this.total&&this.loaded.width(this.total.width()*
...
j=this.container.find(".mejs-volume-slider, .mejs-horizontal-volume-slider"),h=this.container.find(".mejs-volume-total, .mejs-horizontal-volume-total"),r=this.container.find(".mejs-volume-current, .mejs-horizontal-volume-current"),k=this.container.find(".mejs-volume-handle, .mejs-horizontal-volume-handle"),p=function(m,o){if(!j.is(":visible")&&typeof o==="undefined"){j.show();p(m,true);j.hide()}else{m=Math.max(0,m);m=Math.min(m,1);m==0?g.removeClass("mejs-mute").addClass("mejs-unmute"):g.removeClass("mejs-unmute").addClass("mejs-mute");
if(d=="vertical"){var t=h.height(),u=h.position(),v=t-t*m;k.css(Math.round("top",u.top+v-k.height()/2));r.height(Math.round(t-v));r.css("top",Math.round(u.top+v))}else{t=h.width();u=h.position();t=t*m;k.css("left",Math.round(u.left+t-k.width()/2));r.width(Math.round(t))}}},n=function(m){var o=null,t=h.offset();if(d=="vertical"){o=h.height();parseInt(h.css("top").replace(/px/,""),10);o=(o-(m.pageY-t.top))/o;if(t.top==0||t.left==0)return}else{o=h.width();o=(m.pageX-t.left)/o}o=Math.max(0,o);o=Math.min(o,
1);p(o);o==0?e.setMuted(true):e.setMuted(false);e.setVolume(o)},s=false,l=false;g.hover(function(){j.show();l=true},function(){l=false;!s&&d=="vertical"&&j.hide()});j.bind("mouseover",function(){l=true}).bind("mousedown",function(m){n(m);f(document).bind("mousemove.vol",function(o){n(o)}).bind("mouseup.vol",function(){s=false;f(document).unbind(".vol");!l&&d=="vertical"&&j.hide()});s=true;return false});g.find("button").click(function(){e.setMuted(!e.muted)});e.addEventListener("volumechange",function(){if(!s)if(e.muted){p(0);
...
a.isFullScreen?a.exitFullScreen():a.enterFullScreen()});else{var j=null;if(function(){var l=document.createElement("x"),m=document.documentElement,o=window.getComputedStyle;if(!("pointerEvents"in l.style))return false;l.style.pointerEvents="auto";l.style.pointerEvents="x";m.appendChild(l);o=o&&o(l,"").pointerEvents==="auto";m.removeChild(l);return!!o}()&&!mejs.MediaFeatures.isOpera){var h=false,r=function(){if(h){k.hide();p.hide();n.hide();g.css("pointer-events","");d.controls.css("pointer-events",
"");h=false}},k=f('<div class="mejs-fullscreen-hover" />').appendTo(d.container).mouseover(r),p=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(r),n=f('<div class="mejs-fullscreen-hover"  />').appendTo(d.container).mouseover(r),s=function(){var l={position:"absolute",top:0,left:0};k.css(l);p.css(l);n.css(l);k.width(d.container.width()).height(d.container.height()-d.controls.height());l=g.offset().left-d.container.offset().left;fullScreenBtnWidth=g.outerWidth(true);p.width(l).height(d.controls.height()).css({top:d.container.height()-
d.controls.height()});n.width(d.container.width()-l-fullScreenBtnWidth).height(d.controls.height()).css({top:d.container.height()-d.controls.height(),left:l+fullScreenBtnWidth})};f(document).resize(function(){s()});g.mouseover(function(){if(!d.isFullScreen){var l=g.offset(),m=a.container.offset();e.positionFullscreenButton(l.left-m.left,l.top-m.top,false);g.css("pointer-events","none");d.controls.css("pointer-events","none");k.show();n.show();p.show();s();h=true}});e.addEventListener("fullscreenchange",
function(){r()})}else g.mouseover(function(){if(j!==null){clearTimeout(j);delete j}var l=g.offset(),m=a.container.offset();e.positionFullscreenButton(l.left-m.left,l.top-m.top,true)}).mouseout(function(){if(j!==null){clearTimeout(j);delete j}j=setTimeout(function(){e.hideFullscreenButton()},1500)})}a.fullscreenBtn=g;f(document).bind("keydown",function(l){if((mejs.MediaFeatures.hasTrueNativeFullScreen&&mejs.MediaFeatures.isFullScreen()||d.isFullScreen)&&l.keyCode==27)a.exitFullScreen()})}},enterFullScreen:function(){var a=
...
if(!d.start&&c.eq(b-1).attr("end"))d.start=mejs.Utility.convertSMPTEtoSeconds(c.eq(b-1).attr("end"));if(c.eq(b).attr("end"))d.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b).attr("end"));if(!d.stop&&c.eq(b+1).attr("begin"))d.stop=mejs.Utility.convertSMPTEtoSeconds(c.eq(b+1).attr("begin"));if(e){g="";for(var j in e)g+=j+":"+e[j]+";"}if(g)d.style=g;if(d.start==0)d.start=0.2;a.times.push(d);d=f.trim(c.eq(b).html()).replace(/(\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/ig,
...
true},disableContextMenu:function(){this.isContextMenuEnabled=false},contextMenuTimeout:null,startContextMenuTimer:function(){var a=this;a.killContextMenuTimer();a.contextMenuTimer=setTimeout(function(){a.hideContextMenu();a.killContextMenuTimer()},750)},killContextMenuTimer:function(){var a=this.contextMenuTimer;if(a!=null){clearTimeout(a);delete a}},hideContextMenu:function(){this.contextMenu.hide()},renderContextMenu:function(a,b){for(var c=this,e="",d=c.options.contextMenuItems,g=0,j=d.length;g<
j;g++)if(d[g].isSeparator)e+='<div class="mejs-contextmenu-separator"></div>';else{var h=d[g].render(c);if(h!=null)e+='<div class="mejs-contextmenu-item" data-itemindex="'+g+'" id="element-'+Math.random()*1E6+'">'+h+"</div>"}c.contextMenu.empty().append(f(e)).css({top:b,left:a}).show();c.contextMenu.find(".mejs-contextmenu-item").each(function(){var r=f(this),k=parseInt(r.data("itemindex"),10),p=c.options.contextMenuItems[k];typeof p.show!="undefined"&&p.show(r,c);r.click(function(){typeof p.click!=
...
(function(f){function a(b,c,e){for(var d in c.type){q=c.quality[d];if(q===e){b.pause();b.setSrc(c.src[d])}}}f.extend(mejs.MepDefaults,{hdsdText:"HD/SD"});f.extend(MediaElementPlayer.prototype,{buildhdsd:function(b,c,e,d){var g="",j="",h=0,r=true,k={type:[],quality:[],src:[]},p=f('<div class="mejs-button mejs-hd-button mejs-sd" ><button type="button" aria-controls="'+this.id+'" title="'+this.options.hdsdText+'"></button></div>').appendTo(c).delegate(".mejs-hd-button > button","click",function(){j=
d.currentSrc;r=d.paused;h=d.currentTime;for(var n in d.children){src=d.children[n];if(src.nodeName==="SOURCE"&&src.src==j)g=f(src).attr("type")}if(g!==""&&!k.src.length)for(n in d.children){src=d.children[n];if(src.nodeName==="SOURCE"&&f(src).attr("type")==g&&k.src.indexOf(f(src).attr("src"))&&(d.canPlayType(src.type)=="probably"||d.canPlayType(src.type)=="maybe")){k.type.push(f(src).attr("type"));k.src.push(f(src).attr("src"));f(src).attr("data-quality")?k.quality.push(f(src).attr("data-quality")):
k.quality.push("sd")}}if(p.hasClass("mejs-sd")){a(d,k,"hd");p.removeClass("mejs-sd").addClass("mejs-hd")}else{a(d,k,"sd");p.removeClass("mejs-hd").addClass("mejs-sd")}setTimeout(function(){r||f(".mejs-playpause-button").click();d.addEventListener("loadeddata",function(){d.setCurrentTime(h)},false)},250)})}})})(mejs.$);
(function(f){f.extend(mejs.MepDefaults,{sourcechooserText:"Source Chooser"});f.extend(MediaElementPlayer.prototype,{buildsourcechooser:function(a,b,c,e){a.sourcechooserButton=f('<div class="mejs-button mejs-sourcechooser-button"><button type="button" aria-controls="'+this.id+'" title="'+this.options.sourcechooserText+'"></button><div class="mejs-sourcechooser-selector"><ul></ul></div></div>').appendTo(b).hover(function(){f(this).find(".mejs-sourcechooser-selector").css("visibility","visible")},function(){f(this).find(".mejs-sourcechooser-selector").css("visibility",
"hidden")}).delegate("input[type=radio]","click",function(){src=this.value;if(e.currentSrc!=src){currentTime=e.currentTime;paused=e.paused;e.setSrc(src);paused||e.play()}});for(i in e.children){src=e.children[i];if(src.nodeName==="SOURCE"&&(e.canPlayType(src.type)=="probably"||e.canPlayType(src.type)=="maybe"))a.addSourceButton(src.src,src.title,src.type,e.src==src.src)}},addSourceButton:function(a,b,c,e){if(b===""||b==undefined)b=a;c=c.split("/")[1];this.sourcechooserButton.find("ul").append(f('<li><input type="radio" name="'+
...
(function(f){f.extend(mejs.MepDefaults,{googleAnalyticsTitle:"",googleAnalyticsCategory:"Videos",googleAnalyticsEventPlay:"Play",googleAnalyticsEventPause:"Pause",googleAnalyticsEventEnded:"Ended",googleAnalyticsEventTime:"Time"});f.extend(MediaElementPlayer.prototype,{buildgoogleanalytics:function(a,b,c,e){e.addEventListener("play",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventPlay,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+
1)];_gaq.push(ev)}},false);e.addEventListener("pause",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventPause,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+1)];_gaq.push(ev)}},false);e.addEventListener("ended",function(){if(typeof _gaq!="undefined"){ev=["_trackEvent",a.options.googleAnalyticsCategory,a.options.googleAnalyticsEventEnded,a.media.currentSrc.substring(a.media.currentSrc.lastIndexOf("/")+1)];_gaq.push(ev)}},
false)}})})(mejs.$);
